...
class ReviewService(
    ...
    fun findReviews(storeId: Long) =
        entityFinderService.getAllReviews().filter { it.store.id == storeId }.map { ReviewResponse.from(it) }

    // 단일 리뷰 조회
    fun findReview(storeId: Long, reviewId: Long) =
        ReviewResponse.from(entityFinderService.getReview(reviewId))
    ...
    fun addReview(memberPrincipal: MemberPrincipal, storeId: Long, request: ReviewRequest) {
        ...
        reviewRepository.save(
            request.to(
                member = entityFinderService.getMember(memberPrincipal.id),
                store = entityFinderService.getStore(storeId)
            )
        ).id
    }
    ...
    fun updateReview(memberPrincipal: MemberPrincipal, storeId: Long, reviewId: Long, request: ReviewRequest) {
        validateContent(request.content)
        entityFinderService.getReview(reviewId).let {
    ...
    private fun availableToAddComment(memberId: Long, storeId: Long) {
        if (!entityFinderService.getAllReservations().any { it.store.id == storeId && it.member.id == memberId })
            throw InvalidRoleException("Add Review")
        else if (entityFinderService.getStore(storeId).status != StoreStatus.OK)
            throw StoreRequirementDeniedException("review")
        else if (entityFinderService.getMember(memberId).bannedExpiration != null)
            throw BannedUserException(entityFinderService.getMember(memberId).bannedExpiration!!)
    }
